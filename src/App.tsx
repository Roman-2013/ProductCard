...
function App() {
    ...
    const [searchProdact, setsearchProdact] = useState(products)
    ...
    let filterMain= products.filter(item => item.title.toLowerCase().includes(searchvalue.toLowerCase()))

    const filteredItems = (e: KeyboardEvent<HTMLInputElement>) => {
        console.log(e.key)
        if (e.key === 'Enter'|| e.key === 'Backspace') {
            setsearchProdact(filterMain)
        } else {
            return filterMain
        }
    }
    ...
    return (
        <div className="App">
            ...
            <input onKeyDown={filteredItems} value={searchvalue} onChange={onChangeInputHandler}/>
            {searchvalue && <button onClick={onClickDeleteButtonHandler}>X</button>}


            {searchProdact.map(el => {
                return (
